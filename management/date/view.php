...
<h2 class =" heading">Date Information</h2>
...
echo "<table><tr><th>Date_ID</th><th>Starting_Date</th><th>Ending_Date</th><th>Action</th><th> Action</th><th>Action</th></tr>";
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				
				
               echo "<tr><td>"  . $row["Date_ID"]."</td><td>". $row["Starting_Date"]."</td><td>".$row["Ending_Date"]."</td><td><a href='dateedit.php?Date_ID=".$row['Date_ID']."' style='text-decoration: none' />Edit</a></td><td><form action='' method='GET'><input type='submit' name='delete".$row['Date_ID']."' value='Delete'></form></td><td><a href='printdate.php?Date_ID=".$row['Date_ID']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";

				// DELETE CODE STARTS FORM HERE
				if(isset($_GET['delete'.$row['Date_ID']])){
					$delete = "DELETE FROM tb_date WHERE Date_ID=".$row['Date_ID']."";
