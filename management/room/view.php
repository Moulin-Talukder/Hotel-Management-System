...
echo "<table><tr><th>Room_ID</th><th>Room_No</th><th>Type</th><th>Rent</th><th>Action</th><th> Action</th><th>Action</th></tr>";
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				
				
               echo "<tr><td>"  . $row["Room_ID"]."</td><td>". $row["Room_No"]."</td><td>".$row["Type"]."</td><td>". $row["Rent"]."</td><td><a href='roomedit.php?Room_ID=".$row['Room_ID']."' style='text-decoration: none' />Edit</a></td><td><form action='' method='GET'><input type='submit' name='delete".$row['Room_ID']."' value='Delete'></form></td><td><a href='printroom.php?Room_ID=".$row['Room_ID']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";

				// DELETE CODE STARTS FORM HERE
				if(isset($_GET['delete'.$row['Room_ID']])){
					$delete = "DELETE FROM tb_room WHERE Room_ID=".$row['Room_ID']."";
