...
echo "<table><tr><th>Employee_ID</th><th>Employee_Name</th><th>Phone</th><th>Address</th><th>Action</th><th> Action</th><th>Action</th></tr>";
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				
				
               echo "<tr><td>"  . $row["Employee_ID"]."</td><td>". $row["Employee_Name"]."</td><td>".$row["Phone"]."</td><td>". $row["Address"]."</td><td><a href='empedit.php?Employee_ID=".$row['Employee_ID']."' style='text-decoration: none' />Edit</a></td><td><form action='' method='GET'><input type='submit' name='delete".$row['Employee_ID']."' value='Delete'></form></td><td><a href='printemp.php?Employee_ID=".$row['Employee_ID']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";

				// DELETE CODE STARTS FORM HERE
				if(isset($_GET['delete'.$row['Employee_ID']])){
					$delete = "DELETE FROM tb_employee WHERE Employee_ID=".$row['Employee_ID']."";
